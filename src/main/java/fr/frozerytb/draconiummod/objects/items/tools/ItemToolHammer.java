...
public class ItemToolHammer extends ItemPickaxe implements IHasmodel {
    ...
    public boolean canApplyAtEnchantingTable(ItemStack stack, net.minecraft.enchantment.Enchantment enchantment) {
        ...
        if (enchantment == net.minecraft.init.Enchantments.EFFICIENCY) {
            return false;
        }
    ...
    public boolean onBlockDestroyed(ItemStack breaker, World w, IBlockState state, BlockPos pos, EntityLivingBase e) {
        if (e instanceof EntityPlayer && !w.isRemote) {
            if (state.getBlock() != Blocks.STONE) {
                return false; // Ne mine que la pierre
            }
            ...
            if (r != null && r.typeOfHit == RayTraceResult.Type.BLOCK) {
                ...
                if (side == EnumFacing.DOWN || side == EnumFacing.UP) {
                    // Détruire uniquement la pierre dans les positions adjacentes
                    destroyIfStone(w, p, breaker, x + 1, y, z - 1);
                    destroyIfStone(w, p, breaker, x + 1, y, z);
                    destroyIfStone(w, p, breaker, x + 1, y, z + 1);
                    destroyIfStone(w, p, breaker, x, y, z - 1);
                    destroyIfStone(w, p, breaker, x, y, z + 1);
                    destroyIfStone(w, p, breaker, x - 1, y, z - 1);
                    destroyIfStone(w, p, breaker, x - 1, y, z);
                    destroyIfStone(w, p, breaker, x - 1, y, z + 1);
                } else if (side == EnumFacing.NORTH || side == EnumFacing.SOUTH) {
                    destroyIfStone(w, p, breaker, x + 1, y + 1, z);
                    destroyIfStone(w, p, breaker, x, y + 1, z);
                    destroyIfStone(w, p, breaker, x - 1, y + 1, z);
                    destroyIfStone(w, p, breaker, x + 1, y, z);
                    destroyIfStone(w, p, breaker, x - 1, y, z);
                    destroyIfStone(w, p, breaker, x + 1, y - 1, z);
                    ...
                    destroyIfStone(w, p, breaker, x, y + 1, z + 1);
                    destroyIfStone(w, p, breaker, x, y + 1, z);
                    destroyIfStone(w, p, breaker, x, y + 1, z - 1);
                    destroyIfStone(w, p, breaker, x, y, z + 1);
                    destroyIfStone(w, p, breaker, x, y, z - 1);
    ...
    private void destroyIfStone(World w, EntityPlayer p, ItemStack breaker, int x, int y, int z) {
        BlockPos pos = new BlockPos(x, y, z);
        IBlockState blockState = w.getBlockState(pos);

        if (blockState.getBlock() == Blocks.STONE) {
            blockState.getBlock().harvestBlock(w, p, pos, blockState, w.getTileEntity(pos), breaker);
            w.setBlockToAir(pos);
        }
    }
}
